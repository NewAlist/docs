...
稳定版：`luolili233/newalist:latest` 或指定本版，如 `luolili233/newalist:v3.41.0` `luolili233/newalist:v3.40.0`

最新镜像版本，请参阅 https://hub.docker.com/r/luolili233/newalist/tags

开发版：`luolili233/newalist:beta`
...
你可以在上述任意镜像标签后面，使用 `-` 符号附加后缀以切换到附带环境的镜像。如 `luolili233/newalist:latest-aio` `luolili233/newalist:beta-aria2` `luolili233/newalist:v3.40.0-ffmpeg`
...
Q：我的版本是v3.x.x 怎么也升级不到最新版 `docker pull luolili233/newalist:latest`拉取最新不行 改成docker-compose安装还是3.x.x版本

A：原因是你的docker设置了镜像，从镜像更新不到最新版本，改一下/etc/docker/daemon.json，删除"registry-mirrors": ["镜像加速器地址"]

- 删除若不行，可以考虑更换一个`镜像加速地址`
- 或者简单粗暴：下载时将`luolili233/newalist:latest` 替换为`luolili233/newalist:v3.16.3`（指定版本，写教程时最新的是3.16.3）
